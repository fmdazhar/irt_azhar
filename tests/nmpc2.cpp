...
// #include "ipopt/AutoDiffScalar.h"
...
public:
  ...
  T obj_func(const Eigen::Matrix<T, 5, 1>& xu) const {
        ...
        T state_cost = (xu.head(3).transpose() * Qm * xu.head(3)).value();
        T control_cost = (xu.tail(2).transpose() * Rm * xu.tail(2)).value();
        T terminal_cost = (xu.head(3).transpose() * QNm * xu.head(3)).value();
...
private:
  hessian_t hessian_ad(const state_t& xu, int N) {
    ...
    for (int i = 0; i < total_size; ++i) {
      xu_ad(i).value() = xu(i % xu.size());
      xu_ad(i).derivatives().resize(total_size);
      xu_ad(i).derivatives().setZero();
      xu_ad(i).derivatives()(i) = 1.0;
    }

    ad_scalar_t obj_func_ad = problem_.template obj_func<ad_variable_t>(xu_ad);
