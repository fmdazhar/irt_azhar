...
public:
    IpoptInterface()
    {
        ...
        m_app->Options()->SetStringValue("hsllib", "/usr/local/lib/libcoinhsl.so");
//        m_app->Options()->SetStringValue("linear_solver", "ma97");
...
public:
    ...
    void solve(const Eigen::Ref<const nlp_variable_t>& x_guess, const Eigen::Ref<const nlp_dual_t>& lam_guess)
    {
        ...
        if( status != Ipopt::Solve_Succeeded )
        {
            std::cout << "*** Error during initialization!" << status << std::endl;
        }

        m_info.status = status;
        m_info.iter = static_cast<int>(m_app->Statistics()->IterationCount());
        ...
        m_cost = static_cast<scalar_t>(m_app->Statistics()->FinalObjective());
    }


//    void solve(Eigen::Ref<const nlp_variable_t> x_guess = Eigen::Ref<const nlp_variable_t>(nlp_variable_t::Zero(IpoptAdapter<Problem>::num_variables)),
//               Eigen::Ref<const nlp_dual_t> lam_guess = Eigen::Ref<const nlp_dual_t>(nlp_dual_t::Zero(IpoptAdapter<Problem>::num_constraints)))
//    {
//        std::cout << "Starting solve function..." << std::endl;
//
//        // Default to zero vectors if arguments are not provided
//        nlp_variable_t x_guess_local = (x_guess.size() == 0) ? nlp_variable_t::Zero(IpoptAdapter<Problem>::num_variables) : x_guess;
//        nlp_dual_t lam_guess_local = (lam_guess.size() == 0) ? nlp_dual_t::Zero(IpoptAdapter<Problem>::num_constraints) : lam_guess;
//
//        // Ensure initial guesses are valid
//        std::cout << "Expected num_variables: " << IpoptAdapter<Problem>::num_variables << std::endl;
//        std::cout << "Expected num_constraints: " << IpoptAdapter<Problem>::num_constraints << std::endl;
//        std::cout << "Provided x_guess size: " << x_guess_local.size() << std::endl;
//        std::cout << "Provided lam_guess size: " << lam_guess_local.size() << std::endl;
//
//        if (x_guess_local.size() != IpoptAdapter<Problem>::num_variables) {
//            std::cerr << "Error: Initial guess for x has incorrect size." << std::endl;
//            return;
//        }
//        if (lam_guess_local.size() != IpoptAdapter<Problem>::num_constraints) {
//            std::cerr << "Error: Initial guess for lambda has incorrect size." << std::endl;
//            return;
//        }
//
//        std::cout << "Initial guess for x: " << x_guess_local.transpose() << std::endl;
//        std::cout << "Initial guess for lambda: " << lam_guess_local.transpose() << std::endl;
//
//        m_problem->m_x = x_guess_local;
//        m_problem->m_lam = lam_guess_local;
//
//        // Check the problem setup before solving
//        std::cout << "Problem setup:" << std::endl;
//        std::cout << "x: " << m_problem->m_x.transpose() << std::endl;
//        std::cout << "lambda: " << m_problem->m_lam.transpose() << std::endl;
//
//        // Try to solve the problem
//        Ipopt::ApplicationReturnStatus status;
//        status = m_app->OptimizeTNLP(m_problem);
...
//        m_cost = static_cast<scalar_t>(m_app->Statistics()->FinalObjective());
//
//        std::cout << "Solve function finished." << std::endl;
//    }
