...
QpSolverType QpSolverCollection::strToQpSolverType(const std::string & qp_solver_type)
{
  ...
  else if(qp_solver_type == "IPOPT")
  {
    return QpSolverType::IPOPT;
  }
  else
  {
...
bool QpSolverCollection::isQpSolverEnabled(const QpSolverType & qp_solver_type)
{
  ...
  else if(qp_solver_type == QpSolverType::IPOPT)
  {
    return ENABLE_IPOPT;
  }
  else
...
#endif
#if ENABLE_IPOPT
std::shared_ptr<QpSolver> allocateQpSolverIpopt();
#endif
} // namespace QpSolverCollection
